'use client'

import { type ReactNode, type ButtonHTMLAttributes } from 'react'

type ButtonVariant = 'primary' | 'secondary' | 'outline' | 'ghost' | 'danger' | 'whatsapp'
type ButtonSize    = 'sm' | 'md' | 'lg' | 'full'

interface ButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: ButtonVariant
  size?: ButtonSize
  loading?: boolean
  children: ReactNode
  brandColor?: string
  brandColorHover?: string
}

const sizeClasses: Record<ButtonSize, string> = {
  sm:   'px-5 py-2.5 text-sm',
  md:   'px-7 py-3.5 text-base',
  lg:   'px-9 py-4.5 text-lg',
  full: 'w-full px-7 py-3.5 text-base',
}

function Spinner() {
  return (
    <svg
      className="h-4 w-4 animate-spin"
      fill="none"
      viewBox="0 0 24 24"
    >
      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
    </svg>
  )
}

function WhatsAppIcon() {
  return (
    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
    </svg>
  )
}

export function Button({
  variant = 'primary',
  size = 'md',
  loading = false,
  children,
  brandColor,
  brandColorHover,
  className = '',
  disabled,
  style,
  ...props
}: ButtonProps) {
  const isDisabled = disabled || loading

  const baseClasses = [
    'inline-flex items-center justify-center gap-2 font-semibold rounded-[14px]',
    'transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2',
    'select-none',
    sizeClasses[size],
    isDisabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer',
  ].join(' ')

  const variantStyles: Record<ButtonVariant, React.CSSProperties> = {
    primary: {
      background: brandColor ?? '#111827',
      color: '#fff',
      boxShadow: '0 4px 12px rgba(0,0,0,0.15)',
    },
    secondary: {
      background: brandColor ? `${brandColor}15` : '#F3F4F6',
      color: brandColor ?? '#111827',
    },
    outline: {
      background: 'transparent',
      color: brandColor ?? '#111827',
      border: `1.5px solid ${brandColor ?? '#E5E7EB'}`,
    },
    ghost: {
      background: 'transparent',
      color: brandColor ?? '#111827',
    },
    danger: {
      background: '#DC2626',
      color: '#fff',
      boxShadow: '0 4px 12px rgba(220,38,38,0.25)',
    },
    whatsapp: {
      background: '#25D366',
      color: '#fff',
      boxShadow: '0 4px 12px rgba(37,211,102,0.30)',
    },
  }

  return (
    <button
      disabled={isDisabled}
      style={{ ...variantStyles[variant], ...style }}
      className={`${baseClasses} ${className}`}
      onMouseEnter={!isDisabled ? (e) => {
        const el = e.currentTarget as HTMLElement
        if (variant === 'primary') el.style.background = brandColorHover ?? '#1f2937'
        if (variant === 'whatsapp') el.style.background = '#1ebe5d'
        if (variant === 'danger') el.style.background = '#b91c1c'
        if (variant === 'secondary') el.style.background = brandColor ? `${brandColor}25` : '#E5E7EB'
      } : undefined}
      onMouseLeave={!isDisabled ? (e) => {
        const el = e.currentTarget as HTMLElement
        const s = variantStyles[variant]
        el.style.background = (s.background as string) ?? ''
      } : undefined}
      {...props}
    >
      {loading && <Spinner />}
      {variant === 'whatsapp' && !loading && <WhatsAppIcon />}
      {loading ? 'Cargando...' : children}
    </button>
  )
}
